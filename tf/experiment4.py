...
X = tf.placeholder("float", [100, 28, 28, 1])

rbm_w = tf.placeholder("float", [5, 5, 1, 8])
rbm_vb = tf.placeholder("float", [1])
rbm_hb = tf.placeholder("float", [8])

rbm_w_inc = tf.placeholder("float", [5, 5, 1, 8])
rbm_vb_inc = tf.placeholder("float", [1])
rbm_hb_inc = tf.placeholder("float", [8])
...
print(X.get_shape())
print(h0_a.get_shape())
print(rbm_w.get_shape())

# TODO Impossible to do conv_4d_valid_filter in fucking TF

w_positive_grad = tf.nn.conv2d(X, h0_a, strides=[1, 1, 1, 1], padding='VALID')
w_negative_grad = tf.nn.conv2d(v1_a, h1_a, strides=[1, 1, 1, 1], padding='VALID')
...
o_w = np.zeros([8, 1, 5, 5], np.float32)
o_vb = np.zeros([1], np.float32)
o_hb = np.zeros([8], np.float32)

o_w_inc = np.zeros([8, 1, 5, 5], np.float32)
