...
before=`date "+%s"`
CUDA_VISIBLE_DEVICES=-1 python experiment5.py | tee ../results/$exp/$mode/tf/raw_results
after=`date "+%s"`
echo "Time: $((after - before))"
...
before=`date "+%s"`
CUDA_VISIBLE_DEVICES=-1 python experiment5.py | tee ../results/$exp/$mode/keras/raw_results
after=`date "+%s"`
echo "Time: $((after - before))"
...
before=`date "+%s"`
$CAFFE_ROOT/build/tools/caffe train --solver=experiment5_solver.prototxt | tee ../results/$exp/$mode/caffe/raw_results
after=`date "+%s"`
echo "Time: $((after - before))"
...
before=`date "+%s"`
th experiment5.lua | tee ../results/$exp/$mode/torch/raw_results
after=`date "+%s"`
echo "Time: $((after - before))"
